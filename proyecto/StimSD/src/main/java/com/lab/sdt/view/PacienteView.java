...
public class PacienteView implements Serializable {
	...
	//zuri inicio nueva variable
	private String pnombrequipoexp;
	public String getPnombrequipoexp() {
		return pnombrequipoexp;
	}

	public void setPnombrequipoexp(String pnombrequipoexp) {
		this.pnombrequipoexp = pnombrequipoexp;
	}
	...
	//zuri iniicio
	private String pbotonexpmodi;
	
	
	public String getPbotonexpmodi() {
		return pbotonexpmodi;
	}

	public void setPbotonexpmodi(String pbotonexpmodi) {
		this.pbotonexpmodi = pbotonexpmodi;
	}
	...
	private LineChartModel zoomModel;
	 ...
	 private boolean termino_dosis =  false;
		...
		public String presionI;
	     ...
	     private boolean  hay_electroretinografia = false;
		private boolean hay_presionintraocular = false;
		private boolean cambio_fecha = false;
		private boolean hay_peso = false;
	     ...
	     private int electroretinografiaMuestra = 0;
	     ...
	     private int bder;
	     private Date fechaE;
	     private Electroretinografia electro;
		...
		private String direccion_paciente;
	     private String cuenta_paciente;
...
///----------- zuri ----------------
	private Date generarFecha(Date fecha){
		Calendar cal = Calendar.getInstance();
		Calendar tiempo_c = Calendar.getInstance();
		cal.setTime(fecha);
			cal.set(Calendar.HOUR,tiempo_c.getTime().getHours());
			cal.set(Calendar.MINUTE, tiempo_c.getTime().getMinutes());
			cal.set(Calendar.SECOND,tiempo_c.getTime().getSeconds());

		return cal.getTime();
	}
	@SuppressWarnings("deprecation")
	public void cambiar_fecha() {
		cambio_fecha = true;
		setFechaE(generarFecha(getFechaE()));
		RequestContext.getCurrentInstance().execute("PF('nueva_fecha').hide();");
	}
	public void muestra_estudio() {
		craegrafica_Estudio(tipo_estudio) ;
		
	}
	public void regresa_consul() {
		tipos_vistas = 3;
	}
	public void historico_consulta () {
		if(idexp>0) {
			tipos_vistas = 4;
 		}else {
 			 MensajeG.mostrar("Debe seleccionar un expediente", FacesMessage.SEVERITY_ERROR);
 		}
		
	}
	  public void guardar_electro() {
		    electro  = new Electroretinografia();
		    electro.setAder(ader);
		    electro.setAizq(aizq);
		    electro.setBdder(bder);
		    electro.setBizq(bizq);
		    electro.setFechae(new Date());
		    hay_electroretinografia = true;
	  }
	      ...
	      //asignacion de expediente
	         ...
	         public void seleccionar_consulta() {
	        	 try {
		        	 ...
		        	 hay_electroretinografia = false;
		        	 hay_presionintraocular = false; 
		        	 electroretinografiaMuestra = 0;
		        	 presionMuestra = 0;
		        	if(id_consultaM>0) {
		        		 ...
		        		 try {
		        			 electro  = new Electroretinografia();
		        			 electro = u_pacientesService.encuentraelec(id_consultaM);
		        			 setAder(electro.getAder());
		        			 setAizq(electro.getAizq());
		        			 setBder(electro.getBdder());
		        			 setBizq(electro.getBizq());
		        			 if(electro.getIdelectro()>0) {
		        				 hay_electroretinografia = true;
		        				 electroretinografiaMuestra = electro.getIdelectro();
		        			 }
	     			  ...
	     			  try {
	 	               ...
	 	               if(id_consultaM>0) { // consulta modificada
	 	            	  id_con_ex = id_consultaM;
	 	            	 	...
	 	            	 	try {
	 	            	 		if(hay_presionintraocular == true) {
	 	            	 			...
	 	            	 			if(presionMuestra >0) {
	 	            	 				...
	 	            	 				if(cambio_fecha==true) {
	 	            	 					presionintraocular.setFecha(getFechaE());
	 	            	 				}
	 	            	 	...
	 	            	 	try {
	 	            	 		if(hay_electroretinografia == true) {
	 	            	 			//hay_electroretinografia = false;
	 	            	 			if(electroretinografiaMuestra>0) {
		 	            			    electro.setAder(ader);
		 	            			    electro.setAizq(aizq);
		 	            			    electro.setBdder(bder);
		 	            			    electro.setBizq(bizq);
		 	            				if(cambio_fecha==true) {
		 	            					electro.setFechae(getFechaE());
	 	            	 				}
		 	            			    //electro.setFechae(new Date());
		 	            			   electro.setIdelectro(electroretinografiaMuestra);
		 	            			  electro.setIdexpconsulta(id_con_ex);
		 	            			  u_pacientesService.modificarelectroretinografia(electro);
	 	            	 				...
	 	            	 				electro.setIdexpconsulta(id_con_ex);
	 	            	 				u_pacientesService.insertarelectroretinografia(electro);
	 	            	 			}
	 	            	 		}
	 	            	 	}catch(Exception e) {}
...
public String conviertetipouser(String partes) {
	...
	public void asignacion_paciente() {
		...
		String estado_paci = estadoUnidad.estado_por_id(seleccion_paciente.getIdestado()).getEstado();
	     setTelefono_paciente(seleccion_paciente.getTelefono());
	     setEmail_paciente(seleccion_paciente.getEmail());
	     setDireccion_paciente("Calle: "+seleccion_paciente.getCalle() +", Número: "+seleccion_paciente.getNum()+", Colonia: "
	     +seleccion_paciente.getColonia()+ ", C.P: "+seleccion_paciente.getCodigopostal()+", Estado: "+estado_paci);
	     setCuenta_paciente(seleccion_paciente.getCuenta());
	     setContrasena_paciente(seleccion_paciente.getContrasenia());
	     setEdad(seleccion_paciente.getEdad());
	     setSexo(seleccion_paciente.getSexo());
	...
	//zuri inicio 
	public void asignacion_equipoexp() {
		idequipoex = selecion_estimulador.getIdequipo();
		cargar_parametros_e();
			setPnombrequipoexp(convertirserie(selecion_estimulador.getNoserie())+"  "+convertirgeneracion(selecion_estimulador.getNoserie()));
	...
	public void vista_modifica_expedeinte() {
		if(seleccion_expediente != null) {
			...
			MensajeG.mostrar("Seleccione un expediente", FacesMessage.SEVERITY_WARN);
		}
	}
		...
		if(empaquetada_expediente()){
				...
				try {
					boolean existe = false;
						 ...
						 if(existe == true) {
							...
							termino_dosis = false;
							MensajeG.mostrar("Expediente: "+expediente_g.getNombre()+ " con folio "+expediente_g.getNoexpediente() +" Registrado", FacesMessage.SEVERITY_INFO);
						 }
			...
			}else 	if(tipos_vistas == 2)// se modifica expediente
			{
				...
				try {
					...
					paciente_mod.setSexo(sexo);
					consultaUsuarios.actualizaUsuario(paciente_mod);
					MensajeG.mostrar("Expediente: "+expediente_g.getNombre()+ " con folio "+expediente_g.getNoexpediente() +" Modificado", FacesMessage.SEVERITY_INFO);
				}catch(Exception e) {
					MensajeG.mostrar(e.toString()+" id: "+expediente_g.getIdexpediente(),FacesMessage.SEVERITY_INFO);
				}
			}
				 ...
				 if(idpacienteex>0) {
					 if(idenfermedadex>0) {
						 if(idmedicoex>0) {
							 ...
							 MensajeG.mostrar("Seleccione Médico", FacesMessage.SEVERITY_ERROR);
						 }
					 }else {
	...
	private void craegrafica_Estudio(String tipo_estudio1 ) {
	      dateModel = new LineChartModel();
	        LineChartSeries series1 = new LineChartSeries();
	        if(tipo_estudio1.equals("presionintraocular")) {
	        	
	        	series1.setLabel("Ojo Izquierdo");
	        	LineChartSeries series2 = new LineChartSeries();
	            series2.setLabel("Ojo Derecho");
	        	List<Presionintraocular> preciones =  u_pacientesService.encuentra_presiones(idexp);

	        	for(int i = 0;i <=preciones.size()-1;i++) {
	        		int valor_izq = Integer.parseInt(preciones.get(i).getpIzq());
	        		int valor_der = Integer.parseInt(preciones.get(i).getpDer());
	        		series1.set(preciones.get(i).getFecha(),valor_izq);
	        		series2.set(preciones.get(i).getFecha(),valor_der);
	        	}
	        	dateModel.addSeries(series1);
        		dateModel.addSeries(series2);
        		dateModel.setTitle("Gráfica Presión Intraocular");
     	        dateModel.setZoom(true);
     	        dateModel.getAxis(AxisType.Y).setLabel("mmHg");
     	        dateModel.setLegendPosition("e");
     	        DateAxis axis = new DateAxis("Fechas");
     	        axis.setTickAngle(-50);
     	        //axis.setMax("2022-02-01 14:38:29");
     	        axis.setTickFormat("%b %#d, %y");
     	 
     	        dateModel.getAxes().put(AxisType.X, axis);
	         }else if(tipo_estudio1.equals("electroretinografia")) {
	        	 series1.setLabel("Ojo Izquierdo A");
		         LineChartSeries series2 = new LineChartSeries();
		         series2.setLabel("Ojo Derecho A");
		         LineChartSeries series3 = new LineChartSeries();
		         series3.setLabel("Ojo Izquierdo B");
		         LineChartSeries series4 = new LineChartSeries();
		         series4.setLabel("Ojo Derecho B");
		         
		         List<Electroretinografia> electro_es = u_pacientesService.encuentra_electroretinografia(idexp);
		         for(int i = 0;i <=electro_es.size()-1;i++) {
		        	series1.set(electro_es.get(i).getFechae(),electro_es.get(i).getAizq());
		        	series2.set(electro_es.get(i).getFechae(),electro_es.get(i).getAder());
		        	series3.set(electro_es.get(i).getFechae(),electro_es.get(i).getBizq());
		        	series4.set(electro_es.get(i).getFechae(),electro_es.get(i).getBdder());
		         }
		         System.out.println("electro_es: "+electro_es.size());
		         dateModel.addSeries(series1);
	        	 dateModel.addSeries(series2);
	        	 dateModel.addSeries(series3);
	        	 dateModel.addSeries(series4);
	        		
	        	 dateModel.setTitle("Gráfica electroretinografía");
	     	        dateModel.setZoom(true);
	     	        dateModel.getAxis(AxisType.Y).setLabel("mV");
	     	        dateModel.setLegendPosition("e");
	     	        DateAxis axis = new DateAxis("Fechas");
	     	        axis.setTickAngle(-50);
	     	        //axis.setMax("2020-02-01");
	     	        //axis.setMin("2019-02-01");
	     	        axis.setTickFormat("%b %#d, %y");
	     	       dateModel.getAxes().put(AxisType.X, axis);
	         }else if(tipo_estudio1.equals("presion"))  {
	        	 List<ExpedienteConsulta> todo_lo_demas = u_pacientesService.encuentra_consultas(idexp);
	        	 series1.setLabel("Mayor");
		        	LineChartSeries series2 = new LineChartSeries();
		            series2.setLabel("Menor");
		            for(int i = 0;i <=todo_lo_demas.size()-1;i++) {
		            	try{
		            		int mayor = Integer.parseInt(todo_lo_demas.get(i).getpMayor());
			        		int menor = Integer.parseInt(todo_lo_demas.get(i).getpMenor());
			        		series1.set(todo_lo_demas.get(i).getFechaconsulta(),mayor);
			        		series2.set(todo_lo_demas.get(i).getFechaconsulta(),menor);
		            	}catch(Exception e) {}
		        		
		        	}
		        	dateModel.addSeries(series1);
	        		dateModel.addSeries(series2);
	        		 dateModel.setTitle("Gráfica Presio Arterial");
	     	        dateModel.setZoom(true);
	     	        dateModel.getAxis(AxisType.Y).setLabel("mmHg");
	     	        dateModel.setLegendPosition("e");
	     	        DateAxis axis = new DateAxis("Fechas");
	        	 ...
	        	 for(int i = 0;i <=todo_lo_demas.size()-1;i++) {
	        		 if(tipo_estudio1.equals("peso")) {
	        			 series1.setLabel("Peso");
		        			int peso  = 0;
		        	 		try{
		        	 			peso = Integer.parseInt(todo_lo_demas.get(i).getPeso());
		        	 			series1.set(todo_lo_demas.get(i).getFechaconsulta(),peso);
		        	 		}catch(Exception e) {}
		        	 		
	        		 }else if(tipo_estudio1.equals("temperatura")) {
	        			 series1.setLabel("temperatura");
	        			 int temperatura  = 0;
		        	 		try{
		        	 			temperatura = Integer.parseInt(todo_lo_demas.get(i).getTemperatura());
		        	 			series1.set(todo_lo_demas.get(i).getFechaconsulta(),temperatura);
		        	 		}catch(Exception e) {}
		        	 		
	        		 }else if(tipo_estudio1.equals("masa")) {
	        			 series1.setLabel("masa");
	        			 int masa  = 0;
		        	 		try{
		        	 			masa = Integer.parseInt(todo_lo_demas.get(i).getMasaCorp());
		        	 			series1.set(todo_lo_demas.get(i).getFechaconsulta(),masa);
		        	 		}catch(Exception e) {}
		        	 		
	        		 }else if(tipo_estudio1.equals("grasa")) {
	        			 series1.setLabel("grasa");
	        			 int grasa  = 0;
		        	 		try{
		        	 			grasa = Integer.parseInt(todo_lo_demas.get(i).getGrasaCorp());
		        	 			series1.set(todo_lo_demas.get(i).getFechaconsulta(),grasa);
		        	 		}catch(Exception e) {}
		        	 		
		        	 		
	        		 }
	        	 		//System.out.println("electro_es: "+peso);
	        	 		//System.out.println("fecha: "+pesos1.get(i).getFechaconsulta());
		        	}
	        	   	dateModel.addSeries(series1);
	       
	     	        dateModel.setZoom(true);
	     	       if(tipo_estudio1.equals("peso")) {
	     	   		dateModel.setTitle("Gráfica Peso");
	     	    	  dateModel.getAxis(AxisType.Y).setLabel("Kg");
	     	       }else if(tipo_estudio1.equals("temperatura")) {
	     	    	  dateModel.setTitle("Gráfica Temperatura");
	     	    	  dateModel.getAxis(AxisType.Y).setLabel("ºC"); 
	     	       }else if(tipo_estudio1.equals("masa")) {
	     	    	  dateModel.setTitle("Gráfica Masa Corporal");
	     	    	  dateModel.getAxis(AxisType.Y).setLabel("%");
	     	       }else if(tipo_estudio1.equals("grasa")) {
	     	    	  dateModel.setTitle("Gráfica Grasa Corporal");
	     	    	  dateModel.getAxis(AxisType.Y).setLabel("%");
	     	       }
	...
	public int getAizq() {
			...
			public boolean isHay_electroretinografia() {
						return hay_electroretinografia;
					}

			public void setHay_electroretinografia(boolean hay_electroretinografia) {
						this.hay_electroretinografia = hay_electroretinografia;
					}
			...
			public String getTipo_estudio() {
				...
				public void setDateModel(LineChartModel dateModel) {
			     ...
			     public boolean isCambio_fecha() {
						return cambio_fecha;
					}

					public void setCambio_fecha(boolean cambio_fecha) {
				     ...
				     public String getTelefono_paciente() {
						...
						public String getCuenta_paciente() {
							return cuenta_paciente;
						}

						public void setCuenta_paciente(String cuenta_paciente) {
							this.cuenta_paciente = cuenta_paciente;
						}
