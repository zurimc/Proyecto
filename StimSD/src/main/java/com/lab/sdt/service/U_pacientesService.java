...
import com.lab.sdt.dao.ElectroretinografiaMapper;
...
public class U_pacientesService {
	...
	@Autowired
	private ElectroretinografiaMapper electroretinografiaMapper;
...
public void registroPresion(Presionintraocular presI) throws Exception{
	...
	public void insertarelectroretinografia(Electroretinografia electroretinografia)throws Exception{
		electroretinografiaMapper.insert(electroretinografia);
	}
	public void modificarelectroretinografia(Electroretinografia electroretinografia)throws Exception{
		electroretinografiaMapper.updateByPrimaryKey(electroretinografia);
	}
...
public Electroretinografia encuentraelec (int idexpC) {
	ElectroretinografiaExample ex_pre = new ElectroretinografiaExample();
	com.lab.sdt.model.ElectroretinografiaExample.Criteria criPre = ex_pre.createCriteria();
	criPre.andIdexpconsultaEqualTo(idexpC);
	List<Electroretinografia> exp_elec = electroretinografiaMapper.selectByExample(ex_pre);
	return exp_elec.get(0);
}
public List<ExpedienteConsulta>  encuentra_consultas(int idexp){
	ExpedienteConsultaExample exp_ex = new ExpedienteConsultaExample();
	Criteria critexp = exp_ex.createCriteria();
	critexp.andIdexpedienteEqualTo(idexp);
	List<ExpedienteConsulta> expconl =  expedienteConsultaMapper.selectByExample(exp_ex);
...
public List<Electroretinografia> encuentra_electroretinografia(int idexp){
	ExpedienteConsultaExample exp_ex = new ExpedienteConsultaExample();
	List<Electroretinografia> electro = new ArrayList<Electroretinografia>();
	Criteria critexp = exp_ex.createCriteria();
	critexp.andIdexpedienteEqualTo(idexp);
	List<ExpedienteConsulta> expconl =  expedienteConsultaMapper.selectByExample(exp_ex);
	for(int i = 0;i <=expconl.size()-1;i++) {
		int id_expecon = expconl.get(i).getIdexpeconsul();
		try {
			if(id_expecon>0) {
				Electroretinografia  elec = new Electroretinografia();
				elec = encuentraelec(id_expecon);
				electro.add(elec);
			}
...
public List<Presionintraocular> encuentra_presiones (int idexp) {
	List<Presionintraocular> entrega = new ArrayList<Presionintraocular>();
	ExpedienteConsultaExample exp_ex = new ExpedienteConsultaExample();
	Criteria critexp = exp_ex.createCriteria();
	critexp.andIdexpedienteEqualTo(idexp);
	List<ExpedienteConsulta> expconl =  expedienteConsultaMapper.selectByExample(exp_ex);
	for(int i = 0;i <=expconl.size()-1;i++) {
		...
		try {
			if(id_expecon>0) {
				Presionintraocular agrega = new Presionintraocular(); 
				agrega = encuentrap(id_expecon);
				entrega.add(agrega);
			}
