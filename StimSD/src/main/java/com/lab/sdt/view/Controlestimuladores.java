...
public class Controlestimuladores implements Serializable {
	...
	public void encontrar_fechaEx() {
		try {
			his_man.clear();
			his_man = estimuladorService.por_fecha(selecion_esth.getIdequipo(),fechaI,fechaF);
		 } catch(Exception e) {
			 MensajeG.mostrar(fechaI.toString()+": " +fechaF.toString() +" Error: "+e.toString(), FacesMessage.SEVERITY_ERROR);
		 }
	 ...
	 public void actualizar_fechas() {
		 setFechaI(getFechaI());
		 setFechaF(getFechaF());
		 MensajeG.mostrar(getFechaI().toString(), FacesMessage.SEVERITY_ERROR);
	 }
