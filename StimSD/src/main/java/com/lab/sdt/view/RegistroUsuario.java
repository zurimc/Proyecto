...
private boolean puede_cambiar = false; 
...
private boolean puede_cambiar_operador = false;

private int tipo_cuenta =  0;
@PostConstruct
public void init(){
	setCuenta("");
	...
	Usuario user_pro = gestiondeSeccionView.obtenerUsuarioEnSesion(); 
	tipo_cuenta = user_pro.getIdtipo();
	tipo_cuenta_1() ;
	try {
		...
		if(tipo_cuenta == 5) {
			tipoUsuarios.clear();
			tipoUsuarios = consultaUsuarios.lista_tipousuarios_operador();
		}
		cuentas1 = consultaUsuarios.lista_cuentas(tipo_cuenta);
		
	}catch(Exception e) {
		MensajeG.mostrar(e.toString(), FacesMessage.SEVERITY_WARN);
	}
		//if(puede_cambiar == true)
	
	
	if(user_pro.getIdtipo() == 1) {
		puede_cambiar = true;
	}else {
		puede_cambiar = false;
	}
	if(user_pro.getIdtipo() == 1 || user_pro.getIdtipo() == 5) {
		puede_cambiar_operador = true;
	}else {
		puede_cambiar_operador = false;
	}
	seleccion_user = user_pro;
	carga_datos_user();
}
...
public void especiladiad() {
	...
	try {
		if(consultaUsuarios.cuenta_registro(getCuenta1())==null) {
			...
			user_a = consultaUsuarios.encuentra_cuenta(getCuenta1(),tipo_cuenta).getIdusuario();
	...
	try {
		if(getCuenta1().trim().length() > 0) {
			if(consultaUsuarios.cuenta_registro(getCuenta1())==null) {
				...
				user_a = registroUsuario(1,consultaUsuarios.encuentra_cuenta(getCuenta1(),tipo_cuenta).getIdusuario());
				if(isIngresa_grado()) {registroespecialidad();}
	 ...
	 if(cuenta.trim().length() > 0) {
		 try {
			 ...
			 cuentas1.clear();
				 ...
				 cuent =  consultaUsuarios.encuentra_cuenta(cuenta,tipo_cuenta);
				 if(cuent!=null) {
...
public boolean isPuede_cambiar() {
	return puede_cambiar;
}

public void setPuede_cambiar(boolean puede_cambiar) {
	this.puede_cambiar = puede_cambiar;
}
...
public boolean isPuede_cambiar_operador() {
	return puede_cambiar_operador;
}

public void setPuede_cambiar_operador(boolean puede_cambiar_operador) {
	this.puede_cambiar_operador = puede_cambiar_operador;
}
}